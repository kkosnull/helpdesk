...
echo "<hr>";
			   			while ($row = mysql_fetch_array($result)) {
							...
							$sql_images="select * from  `auto`.`images` where `defect_id` =".$row['id']."";
...
// $con = mysql_connect("db13.grserver.gr", $this->user, $this->password);
				if (!$con)
				  ...
				  else
				  {
					...
					$sql="SELECT* FROM `auto`.`eobd` ORDER BY `id` DESC";
...
// $con = mysql_connect("db13.grserver.gr", $this->user, $this->password);
				if (!$con)
				  ...
				  else
				  {
					...
					$sql="SELECT a.id, a.code, a.description, b.brand FROM `auto`.`codes_manufacturers` a inner join `auto`.`brands` b 
							on a.brand_id=b.id
						...
						// select brands
						$sql_brands="SELECT * FROM `auto`.`brands` ORDER BY `brand` ASC";
...
<input type='text' class='hover' style='width:90%; float:left;  border:0px solid; cursor:pointer;' value='".$row['brand']." ".$row['model'].", Χρονιά : ".$row['prod_year'].", Κωδικός κινητήρα : ".$row['engine_serial']."' readonly onClick='document.getElementById(".$row["modelid"].").submit();'>			
...
foreach ($files as &$filename) {
    ...
    $sql_images="INSERT INTO `auto`.`images` (`defect_id`, `file`) VALUES (".$this->defect_id.", '".$filename."')";
