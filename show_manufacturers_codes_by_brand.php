...
// $con = mysql_connect("db13.grserver.gr", $this->user, $this->password);
				if (!$con)
				  ...
				  else
				  {
					...
					$sql="SELECT a.id, a.code, a.description, b.brand FROM `helpdesk`.`codes_manufacturers` a inner join `auto`.`brands` b 
							on a.brand_id=b.id
							WHERE a.brand_id=".$id."
						...
						echo "<input placeholder='Αναζήτηση με κωδικό' id='box_manufact_code' type='text' class='hover' style='width:100%'/>";
